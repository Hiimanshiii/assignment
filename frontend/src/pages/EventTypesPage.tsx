  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-lg font-semibold text-slate-900">Event Types</h1>
      ...
      {isLoading ? (
        <div className="grid gap-3">
          {Array.from({ length: 3 }).map((_, idx) => (
            <div
              key={idx}
              className="h-24 animate-pulse rounded-2xl bg-white ring-1 ring-slate-200"
            />
          ))}
        </div>
      ) : (
      ...
      >
        <div className="space-y-4">
          {formError ? (
            <div className="rounded-xl bg-rose-50 p-3 text-sm text-rose-700 ring-1 ring-rose-100">
              {formError}
            </div>
          ) : null}
